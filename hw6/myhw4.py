...
for i in range(testDataNum):
  ...
  if i == 0:
    print(a.shape)
  image_val[i,:,:,:] = a
label_val = allData.get("testLabels")[:,5000].reshape(testDataNum)
...
image_val = np.zeros((testDataNum, 28, 28, 1))
for i in range(testDataNum):
  a = allData.get("testImages")[:,:,:,i+5000]
  if i == 0:
    print(a.shape)
  image_val[i,:,:,:] = a
label_val = allData.get("testLabels")[testDataNum:testDataNum+testDataNum].reshape(testDataNum)
# label_val = np.ndarray.flatten(label_val)
...
val_accuracy, val_loss = sess.run([accuracy, loss1], feed_dict={inputs: image_val, labels: label_val})
print("ConvNet Hard Validation Accuracy: ", val_accuracy)